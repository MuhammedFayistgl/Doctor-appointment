...
function DoctorsList() {
    ...
    const changeDoctorStatus = async (record:DoctorsType, status:string) => {
        try {
            dispatch(showLoading());
            const resposne = await AxiosConnection.post("/api/admin/change-doctor-account-status", {
                doctorId: record._id,
    ...
    const columns: TableColumnsType<DoctorsType> = [
        {
            title: "Name",
            dataIndex: "name",
            render: (text, record) => (
    ...
    return (
        <Layout>
            <h1 style={{ color: "#0087ba" }} className="page-header">
                <GiStethoscope /> Doctors List
            </h1>
            <hr />
